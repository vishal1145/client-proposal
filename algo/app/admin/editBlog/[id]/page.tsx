...
<div>
  ...
  {blogData.afterAdditionalImage.map((text, index) => (
    <div key={index} className="flex gap-2 mb-2">
      <textarea
        value={text}
        onChange={(e) => {
          const newTexts = [...blogData.afterAdditionalImage];
          newTexts[index] = e.target.value;
          setBlogData({ ...blogData, afterAdditionalImage: newTexts });
        }}
        className="w-full p-2 text-sm border rounded-md focus:ring-1 focus:ring-blue-500"
        rows={2}
      />
      <button
        type="button"
        onClick={() => {
          const newTexts = blogData.afterAdditionalImage.filter((_, i) => i !== index);
          setBlogData({ ...blogData, afterAdditionalImage: newTexts });
        }}
        className="px-2 py-1 text-xs bg-red-500 text-white rounded-md hover:bg-red-600"
      >
  ...
  <button
    ...
    onClick={() => setBlogData({
      ...blogData,
      afterAdditionalImage: [...blogData.afterAdditionalImage, ""]
    })}
    className="mt-1 px-3 py-1 text-xs bg-green-500 text-white rounded-md hover:bg-green-600"
  >
    + Add Paragraph
